...
/- The four-generator quotient core 8,7,5,5 forces excess at least fifteen.
The repeated quotient-five pair is explicitly required to be distinct. -/
set_option maxHeartbeats 0
namespace QuadEightSevenFiveFive
...
lemma forced_ratio (G : Finset ℕ) (hpos : ∀ g ∈ G, 2 ≤ g) (hprim : Primitive G)
    ...
    (hqa : n / a = 8) (hqc : n / c = 5) (hAC2 : 2 ≤ n / Nat.lcm a c) :
    3 * a = 2 * c := by
  ...
  exact TripleEightSixFive.ratio_eight_five hap hcp hnotac hqa hqc hAC2

lemma matched_pairs_impossible (G : Finset ℕ)
    (hpos : ∀ g ∈ G, 2 ≤ g) (hprim : Primitive G) (n a b c d : ℕ)
    (ha : a ∈ G) (hb : b ∈ G) (hc : c ∈ G) (hd : d ∈ G)
    (hqa : n / a = 8) (hqb : n / b = 7) (hqc : n / c = 5) (hqd : n / d = 5)
    (hAC2 : 2 ≤ n / Nat.lcm a c) (hBD2 : 2 ≤ n / Nat.lcm b d)
    (hAB1 : 1 ≤ n / Nat.lcm a b) (hBC1 : 1 ≤ n / Nat.lcm b c) : False := by
  have hap : 0 < a := by have := hpos a ha; omega
  ...
  have hbd := QuadSevenSevenFiveFive.forced_ratio G hpos hprim n b d hb hd hqb hqd hBD2
  have hna : n < 9 * a := (Nat.div_lt_iff_lt_mul hap).mp (by omega)
  have hdn : 5 * d ≤ n := by simpa [hqd] using Nat.div_mul_le_self n d
  have hlow : 15 * b ≤ 2 * n := by omega
  rcases QuadSevenSixFiveFive.ratios_seven_five hbp hcp hnotbc hqb hqc hBC1 hlow
      with h43 | h32 | h75
  · have hab : 8 * b = 9 * a := by omega
    have hm := TripleNineSixFive.lcm_lower_of_coprime_ratio
      hap hbp (by decide : Nat.Coprime 9 8) hab
    have hz : n / Nat.lcm a b = 0 := Nat.div_eq_of_lt (by omega)
    omega
  · have heq : a = b := by omega
    subst b
    omega
  · have hab : 14 * b = 15 * a := by omega
    have hm := TripleNineSixFive.lcm_lower_of_coprime_ratio
      hap hbp (by decide : Nat.Coprime 15 14) hab
    have hz : n / Nat.lcm a b = 0 := Nat.div_eq_of_lt (by omega)
    omega
...
theorem excess_at_least_fifteen (G : Finset ℕ)
  ...
  rw [hcap87] at hAB
  rw [hcap85] at hAC hAD
  rw [hcap75] at hBC hBD
  rw [hcap55] at hCD
  ...
  have hrowB : n / Nat.lcm b c + n / Nat.lcm b d ≤ 3 := by
    ...
    have hbd := QuadSevenSevenFiveFive.forced_ratio G hpos hprim n b d hb hd hqb hqd (by omega)
    have : c = d := by omega
    exact hcdne this
  ...
  · have hBD2 : 2 ≤ n / Nat.lcm b d := by omega
    ...
    exact matched_pairs_impossible G hpos hprim n a b c d ha hb hc hd
      hqa hqb hqc hqd hAC2 hBD2 hAB1 hBC1
  · have hAD2 : 2 ≤ n / Nat.lcm a d := by omega
    have hBC2 : 2 ≤ n / Nat.lcm b c := by omega
    have hBD1 : 1 ≤ n / Nat.lcm b d := by omega
    exact matched_pairs_impossible G hpos hprim n a b d c ha hb hd hc
      hqa hqb hqd hqc hAD2 hBC2 hAB1 hBD1

#print axioms forced_ratio
#print axioms matched_pairs_impossible
#print axioms excess_at_least_fifteen
end QuadEightSevenFiveFive
