...
/- Exact, scale-independent embedding tests for the quotient pattern 7,7,5. -/
set_option maxHeartbeats 0

namespace SevenSevenFivePatterns
...
def coefficients (u v w : ℕ) : Finset ℕ :=
  (Icc 2 7).image (fun j => j * u) ∪
    (Icc 2 7).image (fun j => j * v) ∪
    (Icc 2 5).image (fun j => j * w)

lemma coefficients_swap (u v w : ℕ) : coefficients u v w = coefficients v u w := by
  simp only [coefficients, union_comm]
...
lemma proper_union_eq_image (n a b c u v w d : ℕ)
  ...
  rw [properSet_eq_scaled_image n a u d 7 ha had hqa,
    properSet_eq_scaled_image n b v d 7 hb hbd hqb,
    properSet_eq_scaled_image n c w d 5 hc hcd hqc]
...
lemma certificate_fifteen_sixteen_twenty : certificate 15 16 20 112 120 0 := by
  unfold certificate
  decide
lemma certificate_nine_ten_twelve : certificate 9 10 12 70 72 15 := by
  unfold certificate
  decide
lemma certificate_twentyone_twenty_twentyeight : certificate 21 20 28 147 160 0 := by
  unfold certificate
  decide
lemma certificate_twentyeight_twentyfive_thirtyfive : certificate 28 25 35 196 200 0 := by
  unfold certificate
  decide
lemma certificate_twentyfour_twentyfive_thirty : certificate 24 25 30 175 180 0 := by
  unfold certificate
  decide

lemma certificate_swap {u v w L U K : ℕ} (h : certificate u v w L U K) :
    certificate v u w L U K := by
  simpa only [certificate, coefficients_swap u v w] using h

#print axioms embedding_of_certificate
#print axioms certificate_fifteen_sixteen_twenty
#print axioms certificate_nine_ten_twelve
#print axioms certificate_twentyone_twenty_twentyeight
#print axioms certificate_twentyeight_twentyfive_thirtyfive
#print axioms certificate_twentyfour_twentyfive_thirty
#print axioms certificate_swap

end SevenSevenFivePatterns
