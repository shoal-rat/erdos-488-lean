...
/- Two distinct quotient-five generators cannot coexist with quotients eight
and six when the number of covered nongenerators is at most thirteen. -/
set_option maxHeartbeats 0
namespace QuadEightSixFiveFive
open AuxiliaryCounterexamples SearchPruning ExcessSix ExcessPair ExcessSeven Finset

lemma card_four_lower (A B C D : Finset ℕ) :
    A.card + B.card + C.card + D.card ≤ (A ∪ B ∪ C ∪ D).card +
      (A ∩ B).card + (A ∩ C).card + (A ∩ D).card +
      (B ∩ C).card + (B ∩ D).card + (C ∩ D).card := by
  have h3 := triple_card_lower A B C
...
theorem excess_at_least_fourteen (G : Finset ℕ)
    ...
    (hqa : n / a = 8) (hqb : n / b = 6) (hqc : n / c = 5) (hqd : n / d = 5) :
  ...
  have hsum := card_four_lower (properSet a n) (properSet b n) (properSet c n) (properSet d n)
  ...
  · have hAB2 : 2 ≤ n / Nat.lcm a b := by omega
    ...
    have hab := TripleEightSixFive.ratio_eight_six hap hbp hnotab hqa hqb hAB2
    have hac := TripleEightSixFive.ratio_eight_five hap hcp hnotac hqa hqc hAC2
    have hz := TripleEightSixFive.last_pair_zero hap hbp hcp hqa hab hac
    omega

#print axioms card_four_lower
#print axioms excess_at_least_fourteen
end QuadEightSixFiveFive
