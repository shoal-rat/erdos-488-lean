...
build=old['build'];rat=idx['rat'];transition_literal=idx['transition_literal']
...
def emit(E,profile,name,folder=Path('research/certificates')):
 ...
 s='''import research.NormalizedShared
...
'''
 ...
 s+=f'\ntheorem {name}_certified : SlackCertified {name}_tree {E} {prof} {name}_tree :=\n  {tree_literal(name,list(range(len(nodes))),True)}\n'
 s+=f'\ntheorem {name}_root : {name}_tree.Contains (initial {prof}) := by\n  apply Or.inl\n  decide +kernel\n'
 s+=f'\ntheorem {name}_safe : ∀ profile : List ℕ, List.Subperm profile {prof} → initial profile = initial {prof} →\n    ∀ s, Reaches {E} profile (initial profile) s → ¬ Bad profile s :=\n  shared_dag_safe {E} {prof} {name}_tree {name}_certified {name}_root\n'
 s+=f'\n#print axioms {name}_safe\nend NormalizedCertificate\n'
 path=folder/(name+'.lean');path.write_text(s)
 stats={'E':E,'profile':profile,'states':len(nodes),'edges':sum(len(n['edges']) for n in nodes),'generation_seconds':seconds,'file':str(path),'sha256':hashlib.sha256(path.read_bytes()).hexdigest(),'mode':'integer_shared_nodes','lean_verified':False}
