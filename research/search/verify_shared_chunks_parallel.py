"""Generate and kernel-check a resumable shared DAG with independent proof workers.
...
ap=argparse.ArgumentParser();ap.add_argument('E',type=int);ap.add_argument('profile_json');ap.add_argument('--name',required=True);ap.add_argument('--chunk-size',type=int,default=200);ap.add_argument('--family',action='store_true');ap.add_argument('--jobs',type=int,default=4);ap.add_argument('--pool-name');ap.add_argument('--pool-size',type=int,default=4);args=ap.parse_args();assert args.jobs>=1 and args.pool_size>=1
...
if args.family:assert all(isinstance(p,list) and p and all(isinstance(q,int) and q>=2 for q in p) for p in profile)
else:assert all(isinstance(q,int) and q>=2 for q in profile)
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
generator='generate_normalized_family.py' if args.family else 'generate_normalized_shared.py'
...
for start in range(0,n,args.chunk_size):
 ...
 a=full.index(f'\ntheorem {name}_cache{start}_correct');b=full.index(f'\ntheorem {name}_cache{end}_correct') if end<n else assembly_start
 body=f'import research.certificates.{data_name}\n'+shared_header+full[a:b]+f'\n#print axioms {name}_node{end-1}_verified\n#print axioms {name}_node{end-1}_slack\nend NormalizedCertificate\n'
...
stats.update({'mode':'integer_family_chunks' if args.family else 'integer_shared_chunks','file':str(path),'sha256':sha(path),'files':source_hashes,'dependency_hashes':deps,'fingerprint':fingerprint,'chunk_size':args.chunk_size,'lean_verified':False})
