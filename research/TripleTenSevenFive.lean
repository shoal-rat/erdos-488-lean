...
/- Quotients ten, seven, five force at least fifteen nongenerators. -/
set_option maxHeartbeats 0
namespace TripleTenSevenFive
open AuxiliaryCounterexamples SearchPruning ExcessSix ExcessPair ExcessSeven Finset

lemma ratio_ten_five_upper {a c n : ℕ} (ha : 0 < a) (hc : 0 < c)
    (hac : ¬ a ∣ c) (hqa : n / a = 10) (hqc : n / c = 5)
    (hcommon : 1 ≤ n / Nat.lcm a c) (hn21 : 21 * a ≤ 2 * n) :
    9 * a = 5 * c := by
...
theorem excess_at_least_fifteen (G : Finset ℕ)
    ...
    (hqa : n / a = 10) (hqb : n / b = 7) (hqc : n / c = 5) :
  ...
  have hcn : 5 * c ≤ n := by simpa [hqc] using Nat.div_mul_le_self n c
  have hnotboth : ¬ (3 ≤ n / Nat.lcm a b ∧ 2 ≤ n / Nat.lcm b c) := by
    rintro ⟨hAB3, hBC2⟩
  ...
  have hcases : 3 ≤ n / Nat.lcm a b ∨ 2 ≤ n / Nat.lcm b c := by omega
  have htriple : (properSet a n ∩ properSet b n ∩ properSet c n).card = 0 := by omega
  rcases hcases with hAB3 | hBC2
  · have hab := QuadTenSevenFourFour.ratio_ten_seven hap hbp hnotab hqa hqb hAB3
    have hbn : 7 * b ≤ n := by simpa [hqb] using Nat.div_mul_le_self n b
    have hn21 : 21 * a ≤ 2 * n := by omega
    have hac := ratio_ten_five_upper hap hcp hnotac hqa hqc hAC1 hn21
    have hxp : 0 < 9 * a := by omega
    have hxn : 9 * a ≤ n := by
      have h10n : 10 * a ≤ n := by simpa [hqa] using Nat.div_mul_le_self n a
      omega
  ...
  · obtain ⟨d, hd, hbd, hcd, _, _⟩ :=
      ExcessEightGeometry.ratio_two_three_of_quotients_seven_five
        hbp hcp hnotbc hnotcb hqb hqc hBC2
    rcases CoreTenSevenFourFourThreeThree.ratios_ten_seven hap hbp hnotab hqa hqb hAB2 with hab | hab
    · omega
    · exact hnotac ⟨2, by omega⟩

#print axioms ratio_ten_five_upper
#print axioms excess_at_least_fifteen
end TripleTenSevenFive
