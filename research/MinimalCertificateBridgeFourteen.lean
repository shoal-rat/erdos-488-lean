import research.ExcessThirteenMain

/- A conditional excess-fourteen interface. Coverage and safety for exact
excess fourteen remain explicit hypotheses; this module does not close them. -/
set_option maxHeartbeats 0

namespace MinimalCertificateBridgeFourteen
...
/-- Exact-fourteen coverage and safety suffice, since excesses through thirteen
...
theorem covered_minimal_safe_profiles_imply_slack_bound
    ...
    (hcoverage : ∀ (G : Finset ℕ) (n : ℕ),
      ...
      multipleCount G n = G.card + 14 →
      2 * multipleCount G n < incidenceCount G n + G.card →
      (∀ g ∈ G, quotientExcess G n ≤ 2 * 14 + (n / g - 1)) →
      quotientProfile G n ∈ profiles)
    (hchecked : ∀ profile ∈ profiles, Safe 14 profile)
  ...
  have he14 : multipleCount H n - H.card ≤ 14 := hHE.trans hE
  by_cases he13 : multipleCount H n - H.card ≤ 13
  · have hb := ExcessThirteen.excess_at_most_thirteen_union_bound H n hHpos hHp hHn he13
    omega
  have heq : multipleCount H n - H.card = 14 := by omega
  have hEe : multipleCount H n = H.card + 14 := by omega
  have hweight14 : ∀ g ∈ H, quotientExcess H n ≤ 2 * 14 + (n / g - 1) := by
    intro g hg
  ...
  have hp := hcoverage H n hHpos hHp hHn hEe hHfail hweight14
  have hr := normalized_reachable H n g0 14 hg0 hmin hHpos hHp hHn hHa he14 hconn
...
/-- This is conditional on the two exact-fourteen certificate inputs above. -/
...
end MinimalCertificateBridgeFourteen
