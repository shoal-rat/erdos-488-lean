...
lemma quotient_four_unique (G : Finset ℕ)
    ...
    (hqa : n / a = 11) (hqb : n / b = 7) (hqc : n / c = 4)
    (hC : (nongenerators G n).card ≤ 13) : c = 5 * d := by
  have hap : 0 < a := by omega
  have hcp : 0 < c := by have := hpos c hc; omega
  have hdn : 22 * d ≤ n := by
    have h : 11 * a ≤ n := by simpa [hqa] using Nat.div_mul_le_self n a
    omega
  have hnd : n < 24 * d := by
    have h := (Nat.div_lt_iff_lt_mul (show 0 < b by omega)).mp (show n / b < 8 by omega)
    omega
  have hma : ∀ t : ℕ, 2 ≤ t → t ≤ 11 → t * (2 * d) ∈ nongenerators G n := by
    intro t ht2 ht11
    have hbound : t * a ≤ n := by
      have h := Nat.mul_le_mul_right a ht11
      have h11 := Nat.div_mul_le_self n a
      omega
...
theorem excess_at_least_fourteen (G : Finset ℕ)
    ...
    (hqa : n / a = 11) (hqb : n / b = 7) (hqc : n / c = 4) (hqf : n / f = 4) :
  ...
  have hratio := ratio_eleven_seven hap hbp hab hqa hqb hcommon
  let d := a / 2
